...
internal class HalfUtils
{
	...
	public static float Unpack(ushort h)
	{
		...
		{
			uintValue = HalfToFloatMantissaTable[(int)HalfToFloatOffsetTable[h >> 10] + (h & 0x3FF)] + HalfToFloatExponentTable[h >> 10]
		};
	...
	static HalfUtils()
	{
		...
		for (int i = 0; i < 256; i++)
		{
			...
			{
				FloatToHalfBaseTable[i] = (ushort)(1024 >> -num3 - 14);
				FloatToHalfBaseTable[i | 0x100] = (ushort)((1024 >> -num3 - 14) | 0x8000);
				FloatToHalfShiftTable[i] = (byte)(-num3 - 1);
