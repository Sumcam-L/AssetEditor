...
internal class VS2013DockPaneStrip : DockPaneStripBase
{
	...
	protected override void OnPaint(PaintEventArgs e)
	{
		...
		if (sw.ElapsedMilliseconds > 1)
			System.Diagnostics.Trace.WriteLine($"[Paint] VS2013DockPaneStrip.OnPaint ({base.DockPane.CaptionText}): {sw.ElapsedMilliseconds}ms");
	}
	...
	private void DrawTabStrip_Document(Graphics g)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			{
				DrawTab(g, base.Tabs[i] as TabVS2013, empty);
			}
		}
		g.SetClip(clip);
		if (base.DockPane.DockPanel.DocumentTabStripLocation != DocumentTabStripLocation.Bottom)
	...
	private void DrawTabStrip_ToolWindow(Graphics g)
	{
		...
		{
			DrawTab(g, base.Tabs[i] as TabVS2013, GetTabRectangle(i));
		}
	}
	...
	{
		Rectangle rectangle = new Rectangle(rect.X + ToolWindowImageGapLeft, rect.Y + rect.Height - ToolWindowImageGapBottom - ToolWindowImageHeight, ToolWindowImageWidth, ToolWindowImageHeight);
		Rectangle rectangle2 = ((PatchController.EnableHighDpi == true) ? new Rectangle(rect.X + ToolWindowImageGapLeft, rect.Y + rect.Height - ToolWindowImageGapBottom - TextFont.Height, ToolWindowImageWidth, TextFont.Height) : rectangle);
		...
		if (base.DockPane.ActiveContent == tab.Content)
		{
			...
			g.DrawLine(base.DockPane.DockPanel.Theme.PaintingService.GetPen(toolWindowBorder), rect.Left, rect.Top, rect.Left, rect.Bottom);
			g.DrawLine(base.DockPane.DockPanel.Theme.PaintingService.GetPen(toolWindowBorder), rect.Left, rect.Bottom - 1, rect.Right, rect.Bottom - 1);
			g.DrawLine(base.DockPane.DockPanel.Theme.PaintingService.GetPen(toolWindowBorder), rect.Right - 1, rect.Top, rect.Right - 1, rect.Bottom);
		...
		else
		{
			...
			g.DrawLine(base.DockPane.DockPanel.Theme.PaintingService.GetPen(toolWindowBorder), rect.Left, rect.Top, rect.Right, rect.Top);
