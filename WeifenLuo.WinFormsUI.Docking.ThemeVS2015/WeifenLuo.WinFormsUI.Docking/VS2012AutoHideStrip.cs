...
internal class VS2012AutoHideStrip : AutoHideStripBase
{
	...
	protected override void OnPaint(PaintEventArgs e)
	{
		...
		if (sw.ElapsedMilliseconds > 1)
			System.Diagnostics.Trace.WriteLine($"[Paint] VS2012AutoHideStrip.OnPaint: {sw.ElapsedMilliseconds}ms");
	}
	...
	private void DrawTabStrip(Graphics g, DockState dockState)
	{
		...
		{
			foreach (TabVS2012 item2 in (IEnumerable<Tab>)item.AutoHideTabs)
			{
				DrawTab(g, item2);
			}
		}
	...
	{
		RectangleF bounds = GetTabOutline((TabVS2012)tab, rtlTransform: true).GetBounds();
	...
	protected Tab TabHitTest(Point ptMouse)
	{
		...
		foreach (DockState dockState in dockStates)
		{
			...
			foreach (Pane item in (IEnumerable<Pane>)GetPanes(dockState))
			{
				foreach (TabVS2012 item2 in (IEnumerable<Tab>)item.AutoHideTabs)
				{
					if (GetTabOutline(item2, rtlTransform: true).IsVisible(ptMouse))
					{
						return item2;
					}
				}
	...
	{
		base.OnMouseMove(e);
		TabVS2012 tabVS = (TabVS2012)TabHitTest(PointToClient(Control.MousePosition));
		if (tabVS != null)
		{
			tabVS.IsMouseOver = true;
			Invalidate();
		}
		if (lastSelectedTab != tabVS)
		{
