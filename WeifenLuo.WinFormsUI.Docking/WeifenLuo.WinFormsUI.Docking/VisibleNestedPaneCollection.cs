...
public sealed class VisibleNestedPaneCollection : ReadOnlyCollection<DockPane>
{
	...
	internal void Refresh()
	{
		...
		{
			DockPane current2 = enumerator.Current;
			NestedDockingStatus nestedDockingStatus2 = current2.NestedDockingStatus;
			current2.Bounds = nestedDockingStatus2.PaneBounds;
	...
	private void Remove(DockPane pane)
	{
		...
		for (int num = base.Count - 1; num > IndexOf(pane); num--)
		{
			...
			{
				if (base[num].NestedDockingStatus.DisplayingPreviousPane == pane)
				{
					dockPane = base[num];
					break;
				}
			}
			else if (base[num].NestedDockingStatus.PreviousPane == pane)
			{
				dockPane = base[num];
				break;
			}
		...
		if (dockPane != null)
		{
			...
			dockPane.NestedDockingStatus.SetDisplayingStatus(isDisplaying: true, nestedDockingStatus.DisplayingPreviousPane, nestedDockingStatus.DisplayingAlignment, nestedDockingStatus.DisplayingProportion);
			for (int num3 = num2 - 1; num3 > IndexOf(dockPane); num3--)
			{
				NestedDockingStatus nestedDockingStatus2 = base[num3].NestedDockingStatus;
				if (PatchController.EnableDisplayingPaneFix == true)
				{
					if (nestedDockingStatus2.DisplayingPreviousPane == pane)
					{
						nestedDockingStatus2.SetDisplayingStatus(isDisplaying: true, dockPane, nestedDockingStatus2.DisplayingAlignment, nestedDockingStatus2.DisplayingProportion);
					}
				}
				else if (nestedDockingStatus2.PreviousPane == pane)
				{
					nestedDockingStatus2.SetDisplayingStatus(isDisplaying: true, dockPane, nestedDockingStatus2.DisplayingAlignment, nestedDockingStatus2.DisplayingProportion);
				}
			}
	...
	private void CalculateBounds()
	{
		...
		for (int i = 1; i < base.Count; i++)
		{
			...
			NestedDockingStatus nestedDockingStatus = dockPane.NestedDockingStatus;
			NestedDockingStatus nestedDockingStatus2 = nestedDockingStatus.DisplayingPreviousPane.NestedDockingStatus;
			...
			if (nestedDockingStatus.DisplayingAlignment == DockAlignment.Left)
			{
				...
				_ = nestedDockingStatus.DisplayingAlignment == DockAlignment.Right;
			Rectangle paneBounds2 = paneBounds;
			Rectangle paneBounds3 = paneBounds;
			Rectangle splitterBounds = paneBounds;
			if (nestedDockingStatus.DisplayingAlignment == DockAlignment.Left)
			{
				paneBounds2.Width = (int)((double)paneBounds.Width * nestedDockingStatus.DisplayingProportion) - dockPane.DockPanel.Theme.Measures.SplitterSize / 2;
				splitterBounds.X = paneBounds2.X + paneBounds2.Width;
				splitterBounds.Width = dockPane.DockPanel.Theme.Measures.SplitterSize;
			...
			{
				paneBounds3.Width = paneBounds.Width - (int)((double)paneBounds.Width * nestedDockingStatus.DisplayingProportion) - dockPane.DockPanel.Theme.Measures.SplitterSize / 2;
			...
			{
				paneBounds2.Height = (int)((double)paneBounds.Height * nestedDockingStatus.DisplayingProportion) - dockPane.DockPanel.Theme.Measures.SplitterSize / 2;
				splitterBounds.Y = paneBounds2.Y + paneBounds2.Height;
				splitterBounds.Height = dockPane.DockPanel.Theme.Measures.SplitterSize;
			...
			{
				paneBounds3.Height = paneBounds.Height - (int)((double)paneBounds.Height * nestedDockingStatus.DisplayingProportion) - dockPane.DockPanel.Theme.Measures.SplitterSize / 2;
