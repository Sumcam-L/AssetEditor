...
internal class VS2005AutoHideStrip : AutoHideStripBase
{
	...
	protected override void OnPaint(PaintEventArgs e)
	{
		...
		Color startColor = base.DockPanel.Theme.Skin.AutoHideStripSkin.DockStripGradient.StartColor;
		Color endColor = base.DockPanel.Theme.Skin.AutoHideStripSkin.DockStripGradient.EndColor;
		LinearGradientMode linearGradientMode = base.DockPanel.Theme.Skin.AutoHideStripSkin.DockStripGradient.LinearGradientMode;
	...
	private void DrawTab(Graphics g, TabVS2005 tab)
	{
		...
		Color startColor = base.DockPanel.Theme.Skin.AutoHideStripSkin.TabGradient.StartColor;
		Color endColor = base.DockPanel.Theme.Skin.AutoHideStripSkin.TabGradient.EndColor;
		LinearGradientMode linearGradientMode = base.DockPanel.Theme.Skin.AutoHideStripSkin.TabGradient.LinearGradientMode;
		...
		Color textColor = base.DockPanel.Theme.Skin.AutoHideStripSkin.TabGradient.TextColor;
		if (dockState == DockState.DockLeftAutoHide || dockState == DockState.DockRightAutoHide)
		{
	...
	private Rectangle GetLogicalTabStripRectangle(DockState dockState, bool transformed)
	{
		...
		if (dockState == DockState.DockLeftAutoHide && count > 0)
		{
			num2 = 0;
			num3 = ((count3 != 0) ? num : 0);
			num4 = base.Height - ((count3 != 0) ? num : 0) - ((count4 != 0) ? num : 0);
		}
		...
		else if (dockState == DockState.DockTopAutoHide && count3 > 0)
		{
			num2 = ((count != 0) ? num : 0);
			num3 = 0;
			num4 = base.Width - ((count != 0) ? num : 0) - ((count2 != 0) ? num : 0);
		}
		else
		{
		...
		Rectangle rectangle = new Rectangle(num2, num3, num4, num);
		if (!transformed)
		{
			return rectangle;
		}
		return GetTransformedRectangle(dockState, rectangle);
	}
	...
	private Rectangle GetTabRectangle(TabVS2005 tab, bool transformed)
	{
		...
		int tabX = tab.TabX;
		int num = logicalTabStripRectangle.Y + ((dockState != DockState.DockTopAutoHide && dockState != DockState.DockRightAutoHide) ? TabGapTop : 0);
		int tabWidth = tab.TabWidth;
		int num2 = logicalTabStripRectangle.Height - TabGapTop;
		if (!transformed)
		{
	...
	protected override IDockContent HitTest(Point point)
	{
		...
		{
			if (!GetLogicalTabStripRectangle(dockState, transformed: true).Contains(point))
			{
				continue;
			}
			foreach (Pane item in (IEnumerable<Pane>)GetPanes(dockState))
			{
				foreach (TabVS2005 item2 in (IEnumerable<Tab>)item.AutoHideTabs)
				{
					if (GetTabOutline(item2, transformed: true, rtlTransform: true).IsVisible(point))
					{
	...
	{
		RectangleF bounds = GetTabOutline((TabVS2005)tab, transformed: true, rtlTransform: true).GetBounds();
