...
public class FileCatelog<TKey> : IFileCatalog<TKey>
{
	private ReaderWriterLockSlim m_fileLock = new ReaderWriterLockSlim(LockRecursionPolicy.SupportsRecursion);

	private IDictionary<TKey, DepotFileInfo> m_files = new Dictionary<TKey, DepotFileInfo>();
	...
	public bool AddOrUpdate(TKey fileKey, DepotFileInfo info)
	{
	...
	public void Remove(TKey fileKey)
	{
	...
	public bool TryGetValue(TKey fileKey, out DepotFileInfo info)
	{
		using (new ScopedReaderLock(m_fileLock))
		{
	...
	public void ForEachKey(Action<TKey> visit)
	{
		TKey[] array = Enumerable.Empty<TKey>().ToArray();
		using (new ScopedReaderLock(m_fileLock))
		{
			array = m_files.Keys.ToArray();
		}
		TKey[] array2 = array;
		foreach (TKey obj in array2)
		{
			visit(obj);
		}
	}
