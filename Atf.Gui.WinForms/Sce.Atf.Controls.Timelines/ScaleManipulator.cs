...
public class ScaleManipulator
{
	...
	protected virtual void DrawManipulator(Graphics g, out RectangleF leftHandle, out RectangleF rightHandle)
	{
		...
		if (IsScaling && ScaleHelper.Mode == ScaleMode.TimePeriod)
		{
			...
			g.DrawLine(pen, num, 0f, num2, 0f);
			g.DrawLine(pen, num, 0f, num, HandleHeight);
			g.DrawLine(pen, num2, 0f, num2, HandleHeight);
			return;
		}
		using Pen pen2 = new Pen(Color, 3f);
		g.DrawLine(pen2, num, 0f, num2, 0f);
		g.DrawLine(pen2, num, 0f, num, HandleHeight);
		g.DrawLine(pen2, num2, 0f, num2, HandleHeight);
	}

	private void owner_Paint(object sender, PaintEventArgs e)
	{
		m_visibleManipulator = TimelineControl.CalculateRange(Owner.EditableSelection, out m_worldMin, out m_worldMax);
		if (m_visibleManipulator)
		{
			...
			if (IsScaling)
			{
				...
				Owner.Renderer.DrawGhosts(ghosts, DraggedSide, transform, Owner.ClientRectangle, graphics);
			}
			Region clip = graphics.Clip;
			Rectangle visibleClientRectangle = Owner.VisibleClientRectangle;
			graphics.Clip = new Region(visibleClientRectangle);
			DrawManipulator(graphics, out m_leftHandle, out m_rightHandle);
			graphics.Clip = clip;
		}
	}
