...
public class SelectionManipulator
{
	...
	protected virtual IEnumerable<TimelinePath> GetRangeOfEvents(TimelinePath begin, TimelinePath end)
	{
		...
		TimelineControl.CalculateRange(begin, out var worldStart, out var worldEnd);
		TimelineControl.CalculateRange(end, out var worldStart2, out var worldEnd2);
	...
	private bool Overlaps(TimelinePath path, float beginTime, float endTime)
	{
		...
		Matrix matrix = TimelineControl.CalculateLocalToWorld(path);
