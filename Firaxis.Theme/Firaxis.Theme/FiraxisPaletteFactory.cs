...
public class FiraxisPaletteFactory : IPaletteFactory
{
	private const string Env = "Environment";

	private XDocument _xml;

	public FiraxisPaletteFactory(byte[] file)
	{
		_xml = XDocument.Load((TextReader)new StreamReader(new MemoryStream(file)));
	}
	...
	public void Initialize(DockPanelColorPalette palette)
	{
		...
		palette.CommandBarMenuPopupHovered.Text = ColorTranslatorFromHtml("CommandBarMenuItemMouseOver", foreground: true);
		...
		palette.MainWindowStatusBarDefault.HighlightText = ColorTranslatorFromHtml("StatusBarHighlight", foreground: true);
	...
	{
		string text = _xml.Root.Element("Theme").Elements("Category").FirstOrDefault((XElement item) => item.Attribute("Name").Value == "Environment")?.Elements("Color").FirstOrDefault((XElement item) => item.Attribute("Name").Value == name)?.Element(foreground ? "Foreground" : "Background").Attribute("Source").Value;
		if (text == null)
		{
