...
public static class PythonOperator
{
	public class attrgetter
	{
		...
		public attrgetter(params object[] attrs)
		{
			if (attrs.Length == 0)
			{
				throw PythonOps.TypeError("attrgetter expected 1 arguments, got 0");
			}
			_names = attrs;
		}
		...
		public object Call(CodeContext context, object param)
		{
			...
			object[] array = new object[_names.Length];
			for (int i = 0; i < _names.Length; i++)
			{
				array[i] = GetOneAttr(context, param, _names[i]);
			}
		...
		private static object GetOneAttr(CodeContext context, object param, object val)
		{
			...
			if (num >= 0)
			{
				object oneAttr = GetOneAttr(context, param, text.Substring(0, num));
				return GetOneAttr(context, oneAttr, text.Substring(num + 1, text.Length - num - 1));
			}
			return PythonOps.GetBoundAttr(context, param, text);
		}
	}
	...
	public class itemgetter
	{
		...
		public object Call(CodeContext context, object param)
		{
			...
			object[] array = new object[_items.Length];
			for (int i = 0; i < _items.Length; i++)
			{
				array[i] = PythonOps.GetIndex(context, param, _items[i]);
			}
			return PythonTuple.MakeTuple(array);
		}
	}
	...
	public class methodcaller
	{
		...
		{
			return $"<operator.methodcaller: {_name}>";
		}

		[SpecialName]
		public object Call(CodeContext context, object param)
		{
			object boundAttr = PythonOps.GetBoundAttr(context, param, _name);
			if (_dict == null)
			{
				return PythonOps.CallWithContext(context, boundAttr, _args);
			}
			return PythonCalls.CallWithKeywordArgs(context, boundAttr, _args, _dict);
		}
	}
	...
	{
		if (!(o is int) && !(o is long) && !(o is double) && !(o is float) && !(o is short) && !(o is uint) && !(o is ulong) && !(o is ushort) && !(o is decimal) && !(o is BigInteger) && !(o is Complex))
		{
			return o is byte;
		}
		return true;
	...
	{
		if (CompilerHelpers.GetType(o) != typeof(PythonType) && !(o is PythonDictionary))
		{
			if (!(o is ICollection) && !(o is IEnumerable) && !(o is IEnumerator) && !(o is IList))
			{
	...
	{
		if (Converter.TryConvertToInt32(o, out var result))
		{
			return result;
		}
