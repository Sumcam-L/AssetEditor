...
internal sealed class InfTree
{
	...
	internal static int inflate_trees_dynamic(int nl, int nd, int[] c, int[] bl, int[] bd, int[] tl, int[] td, int[] hp, ZStream z)
	{
		int[] hn = new int[1];
		int[] v = new int[288];
		int num = huft_build(c, 0, nl, 257, InfTreeUtil.cplens, InfTreeUtil.cplext, tl, bl, hp, hn, v);
		if (num != 0 || bl[0] == 0)
		{
			if (num == -3)
			{
		...
		num = huft_build(c, nl, nd, 0, InfTreeUtil.cpdist, InfTreeUtil.cpdext, td, bd, hp, hn, v);
